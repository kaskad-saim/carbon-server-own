import { createChart } from '../components/chartRendererHourSushilki.js';
import { getLastHoursRange } from '../components/dataUtils.js';
import { dataLabelsSushilka } from '../components/data.js';
...
function initializeChart(parameterType, elements, chartTitle) {
  ...
  const chart = createChart({
    parameterType,
    elements,
    labels: dataLabelsSushilka.temperatures,
    units: dataLabelsSushilka.temperatures.map(() => '°C'),
    yAxisConfig: {
      min: 0,
      max: 600,
      stepSize: 100,
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (elements1.chartCanvas) {
    initializeChart('sushilka1', elements1, 'График температур сушилки №1');
  }
  ...
  if (elements2.chartCanvas) {
    initializeChart('sushilka2', elements2, 'График температур сушилки №2');
  }
