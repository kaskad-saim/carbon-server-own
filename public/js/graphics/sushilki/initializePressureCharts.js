...
function initializeChart(parameterType, elements, chartTitle) {
  ...
  const chart = createChart({
    parameterType,
    elements,
    labels: dataLabelsSushilka.pressures,
    units: dataLabelsSushilka.pressures.map((_, index) => (index < 2 ? 'кгс/см²' : 'кгс/м²')),
    yAxisConfig: {
      min: -20,
      max: 30,
      stepSize: 5,
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (elements1.chartCanvas) {
    initializeChart('sushilka1', elements1, 'График давления/разрежения сушилка №1');
  }
  ...
  if (elements2.chartCanvas) {
    initializeChart('sushilka2', elements2, 'График давления/разрежения сушилка №2');
  }
