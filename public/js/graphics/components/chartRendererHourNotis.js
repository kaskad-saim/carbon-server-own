...
import { displayNamesNotis } from './data.js';
...
export function createChart({
                ...
                labels: {
                  ...
                  generateLabels: function (chart) {
                    return chart.data.datasets.map((dataset, i) => {
                      ...
                      const displayName = displayNamesNotis[dataset.label] || dataset.label;
  ...
  function getValueByLabel(item, label) {
    // Проверяем вложенные объекты (если они есть)
    if (item.notisVR1 && item.notisVR1[label] !== undefined) {
      return item.notisVR1[label];
    }
    if (item.notisVR2 && item.notisVR2[label] !== undefined) {
      return parseFloat(item.notisVR2[label]);
    }
    // Проверяем поле data
