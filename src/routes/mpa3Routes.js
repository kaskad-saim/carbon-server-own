...
// Роут для получения данных MPA3
router.get('/mpa3-data', async (req, res) => {
  try {
    const data = await PechMpa3Model.find().sort({ lastUpdated: -1 }).limit(1); // Получаем последние данные
    if (!data[0]) {
    ...
    console.error('Ошибка при получении данных MPA3:', err);
