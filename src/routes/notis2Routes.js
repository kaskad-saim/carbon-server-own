...
import { Notis2Model } from '../models/notisModel.js';
...
router.get('/notis2-data', async (req, res) => {
  try {
    const data = await Notis2Model.find().sort({ lastUpdated: -1 }).limit(1);
    if (!data[0]) {
    ...
    logger.error('Ошибка при получении данных Notis2:', err);
