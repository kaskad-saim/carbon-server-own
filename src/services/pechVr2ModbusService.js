...
export const readDataVr2 = async () => {
  try {
    ...
    for (const [label, address] of Object.entries(temperatureAddresses)) {
      try {
        const value = Math.round(await readFloat(address, 'VR2'));
    ...
    const levelsVr2 = {
      'В ванне скруббера': {
        value: Math.round((await readFloat(0x002a, 'VR2')) * 10),
        percent: Math.round(await readFloat(0x002a, 'VR2')),
      },
      'В емкости ХВО': {
        value: Math.round((await readFloat(0x003e, 'VR2')) * 60),
        percent: Math.round(await readFloat(0x003e, 'VR2')),
      },
      'В барабане котла': {
        value: Math.round((await readFloat(0x0018, 'VR2')) * 4 - 200),
        percent: Math.round(await readFloat(0x0018, 'VR2')),
      },
    };

    // Чтение давлений
    const pressuresVr2 = {
      'Давление газов после скруббера': ((await readFloat(0x0028, 'VR2')) * 0.25).toFixed(1),
      'Пара в барабане котла': ((await readFloat(0x0026, 'VR2')) * 0.16).toFixed(1),
    };

    // Чтение разрежений
    const vacuumsVr2 = {
      'В топке печи': ((await readFloat(0x0020, 'VR2')) * 0.25 - 12.5).toFixed(1),
      'В котле утилизаторе': ((await readFloat(0x0024, 'VR2')) * -0.25).toFixed(1),
      'Низ загрузочной камеры': ((await readFloat(0x0022, 'VR2')) * -0.25).toFixed(1),
    };

    // Чтение импульсных сигналов
    const imVr2 = {
      'ИМ1 скруббер': (await readFloat(0x0044, 'VR2')) > 1,
      'ИМ2 ХВО': (await readFloat(0x0046, 'VR2')) > 1,
      'ИМ3 аварийный сброс': (await readFloat(0x0048, 'VR2')) > 1,
      'ИМ4 пар в отделение активации': (await readFloat(0x004a, 'VR2')) > 1,
      'ИМ5 котел-утилизатор': Math.round(await readFloat(0x001c, 'VR2')),
    };

    // Чтение данных горелки
    const gorelkaVr2 = {
      'Текущая мощность горелки': Math.max(0, Math.round(await readFloat(0x001a, 'VR2'))),
      'Задание температуры на горелку': Math.round(await readFloat(0x002e, 'VR2')),
    };
